...
function recordFields(node) {
  ...
  for (const property of node.properties) {
    const key = literalRecordKey(property);
    if (fields.has(key)) {
...
function literalRecordKey(property) {
  if (
    ...
    throw new Error("NPC record requires literal data properties");
  }
  const key =
  ...
  if (
    ...
    throw new Error("NPC record requires a safe literal field key");
  }
  return key;
}
...
function collectRecords(context, root, nodes) {
  ...
  for (const statement of root.body) {
    ...
    if (records.has(node.left.name)) {
      throw new Error("NPC record cannot be reinitialized");
    }
    records.set(node.left.name, {
      statement,
      assignment: node,
      fields: recordFields(node.right),
      names: new Map(),
    });
  }
  const occupied = new Set(
    nodes.filter((node) => node.type === "Identifier").map((node) => node.name),
  );
  for (const [name, record] of records) {
    ...
    for (const key of record.fields.keys()) {
      ...
      if (occupied.has(generated)) {
        throw new Error("NPC record binding collision");
      }
...
function validateRecordUses(context, nodes, name, record) {
  ...
  for (const node of nodes) {
    ...
    if (!record.fields.has(key)) {
      record.fields.set(key, identifier(node, "undefined"));
    }
  }
  for (const node of nodes) {
    if (
      ...
      throw new Error(
        "NPC record cannot escape, alias, or shadow its global binding",
      );
    }
  }
}
  ...
  for (const record of records.values()) {
    ...
    for (const [key, value] of record.fields) {
      body.push({
        ...
        expression: {
          ...record.assignment,
          left: identifier(record.assignment.left, record.names.get(key)),
          right: value,
        },
      });
    }
