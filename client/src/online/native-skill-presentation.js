...
export class NativeSkillPresentation {
  ...
  createResources(scene) {
    return new SkillResources(scene, { services: this.owner.services, audio: this.owner.audio.audio, report: (error) => this.owner.report(error) });
  }
  setScene(scene) {
  ...
  async prepareLearned() {
    ...
    await this.resources.prepare(this.learnedRecords(skills), (id) => skills[id].level);
    this.signature = signature;
  }
  learnedRecords(skills) {
    const records = Object.entries(skills).filter(([, rank]) => rank.level > 0).map(([id]) => this.owner.catalog.ui.skills[id]);
    if (records.some((record) => !record)) throw new Error("Server learned skill artwork is unavailable.");
    return records;
  }
  publishBuffs(previous) {
    ...
    for (const effect of this.owner.state.self.effects) {
      ...
      if (previous && effect.kind === "skill" && !this.effects.has(effect.id)) this.use(effect.templateId, this.owner.scene.presentation);
    }
  ...
  async remoteResources(event, skill, rank) {
    if (!Number.isInteger(rank) || rank < 1 || !skill.levels[rank]) throw new Error("The server cast rank has no original skill artwork.");
    let entry = this.remote.get(event.actorId);
    if (!entry) {
      if (this.remote.size >= MAX_REMOTE_RESOURCES) throw new Error("Remote skill artwork budget exceeded.");
      entry = { resources: this.createResources(this.owner.scene), skills: new Map(), preparing: null };
  ...
  async castResources(event) {
    ...
    const rank = self ? this.owner.store.profile.skills[event.skillId]?.level : event.rank;
    if (!Number.isInteger(rank) || rank < 1) throw new Error("Server skill rank is unavailable.");
    const resources = self ? this.resources : await this.remoteResources(event, skill, rank);
  ...
  position(id) {
    ...
    if (!view) return null;
    return { get x() { return view.drawX; }, get y() { return view.drawY; }, get facing() { return view.entity.facing; } };
  }
  ...
  async projectile(event) {
    ...
    if (this.projectiles.size >= MAX_CASTS) throw new Error("Skill cast presentation budget exceeded.");
  ...
  async combat(event) {
    ...
    const cast = await this.castResources(event);
    if (scene !== this.owner.scene || this.owner.destroyed) return;
    if (!this.projectiles.delete(event.actionId)) this.playUse(cast, this.position(event.actorId));
    for (const hit of event.hits) {
      const target = this.owner.entities.find((entry) => entry.id === hit.targetId);
  ...
  update(ms) {
    if (this.resources && this.resources.scene !== this.owner.scene) { this.destroy(); return; }
