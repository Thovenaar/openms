import { inspectionElement, inspectionText } from "../development/state-testing.js";
...
export function createOnlineDevelopment(owner) {
  async function request(action) {
    ...
    owner.agent.assertActive();
    if (generation !== owner.agent.generation) throw new Error("Experiment lease changed while the server request settled.");
    return result;
  }
  function setup(action) {
    if (!action || !SETUP_KINDS.has(action.kind)) {
      throw new Error("Server setup accepts only audited map, preset, spawn, profile or physics actions.");
    }
...
function describe(owner, options = {}) {
  ...
  if (scene.entities.length > 16384) throw new Error("Scene observation exceeds its bound.");
  const entities = options.entityId === undefined
  ...
  return structuredClone({
    ...
    policy: "Audited server edits persist according to server rules. No temporary profile, saved baseline or automatic restoration.",
    actions: ["setup(map|preset|spawn|profile|physics)", "pause(boolean)", "step(1–4 ticks)"],
    scene: { currentMap: scene.currentMap, paused: scene.paused, camera: scene.camera,
      entities, entityCount: scene.entities.length, shown: entities.length },
    connection: owner.transport.snapshot(),
    prediction: owner.prediction.snapshot(),
    native: { quests: owner.model?.presentation?.quests,
...
function formAction(kind, value) {
  ...
  if (!Number.isSafeInteger(number)) throw new Error("Enter an integer server action value.");
  switch (kind) {
    case "map": return { kind, mapId: number };
    case "preset": return { kind, job: number };
    case "spawn": return { kind, templateId: number, count: 1 };
    case "step": return { kind, ticks: number };
    default: throw new Error("Unknown server experiment action");
  }
}
...
export function mountOnlineExperiments(owner, root) {
  ...
  inspectionElement(panel, "p", "Developer realm only. Changes use audited HTTP requests and can persist. No offline temporary profile or automatic rollback is available. The automation API additionally requires an active human-granted agent lease.");
  ...
  apply.type = "button";
  apply.addEventListener("click", (event) => execute(event, () => formAction(select.value, value.value)), options);
  addPauseButtons(controls, execute, options);
  function refresh() {
    controls.disabled = pending || !owner.authorized() || owner.transport.status !== "active";
  }
  refresh();
...
function addPauseButtons(root, execute, options) {
  for (const paused of [true, false]) {
    const button = inspectionElement(root, "button", paused ? "Pause server field" : "Resume server field");
    button.type = "button";
    button.addEventListener("click", (event) => execute(event, () => ({ kind: "pause", paused })), options);
  }
}
