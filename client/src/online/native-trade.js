...
export class NativeTrade {
  ...
  update(event) {
    ...
    this.stores = event.participants.map((id) => {
      ...
      if (!member) {
        throw new Error("Trade participant appearance is unavailable.");
      }
  ...
  subscribe(listener) {
    if (this.listeners.size >= 32) {
      throw new Error("Trade observer budget exceeded");
    }
  ...
  snapshot() {
    const participants = this.stores.map((store, side) => {
      ...
      return {
        ...
        mesos: offer?.mesos ?? 0,
        wallet:
          store.id === this.owner.store.id
            ? this.owner.store.profile.meso
            : null,
        offers,
        locked: Boolean(offer?.confirmed),
        prompting: this.pending,
      };
    });
    return {
      state: this.state,
  ...
  confirm() {
    return this.run({ kind: "trade.confirm", tradeId: this.event.tradeId });
  }
  cancel() {
  ...
  async promptItem(side, proposal) {
    if (this.stores[side]?.id !== this.owner.store.id) {
      return unsupported("acting for another trade participant");
    }
    ...
    if (revision !== this.event.revision) {
      return {
        ok: false,
        reason: "The trade changed while the quantity prompt was open.",
      };
    }
    const action = this.observedOffer();
    if (action.items.some((entry) => entry.itemId === item.uid)) {
      return { ok: false, reason: "This item is already offered." };
    }
  ...
  async promptMesos(side) {
    ...
    const value = await this.owner.ui.prompt({
      kind: "number",
      text: "How many mesos will you offer?",
      value: this.observedOffer().mesos,
      min: 0,
      max: this.owner.store.profile.meso,
      owner: this,
    });
    ...
    if (revision !== this.event.revision) {
      return {
        ok: false,
        reason: "The trade changed while the mesos prompt was open.",
      };
    }
    return this.run({ ...this.observedOffer(), mesos: value });
  }
  sendChat() {
    return unsupported("trade-room chat");
  }
  async run(action) {
    if (this.owner.destroyed || this.owner.trade !== this) {
      return {
        ok: false,
        code: "SESSION_EXPIRED",
        reason: "This server trade is no longer observed.",
      };
    }
    if (this.pending) {
      return { ok: false, reason: "A trade request is pending." };
    }
    this.pending = true;
    this.changed();
    try {
      return await this.owner.request(action, this.event.revision);
    } finally {
      this.pending = false;
      this.changed();
    }
