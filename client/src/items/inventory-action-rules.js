...
/** StringPool0x15c3/0x148a, selected by004f31a5 before the yes/no result6 gate. */
export function itemDropWarning(item, template) {
  if (template.info.accountSharable === 1 && !(item.flags & 1)) {
    return "This item cannot be recovered once dropped. However, It is possible to transfer it to a different character on the same account by using the storage system. \r\n\r\nDo you really want to drop this item?";
  }
  return "This item can't be taken back once \r\nthrown away. Will you still drop it?";
}
