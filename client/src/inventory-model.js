...
export function itemStackLimit(template) {
  ...
  if (inventoryType(template.id) === 1) return 1;
...
function mutableInventory(profile) {
  if (
    ...
    throw profileError(
      ...
      "A mutable schema5 transaction draft is required.",
    );
  }
}
...
function validateAttributeRecord(attributes) {
  ...
  const allowed = ["uid", "id", "count", "slot", "owner", "flags", "expiresAt"];
...
function instanceAttributes(attributes) {
...
function planStackMerges(profile, template, attributes, plan) {
  for (const entry of profile.inventory) {
    ...
    if (
      ...
      !compatible(entry, template.id, attributes)
    ) {
      continue;
    }
...
/** Mutates detached drafts only; explicit UID means one whole preserved instance, never merge. */
export function grantItem(profile, template, count, attributes = {}) {
  ...
  const values = instanceAttributes(attributes);
