...
class ShopTabs {
  constructor(panel, side, changed) {
    ...
    this.selected = panel.owner.restoreTab(
      `Shop.${side}`,
      side === "buy" ? [0, 1] : [0, 1, 2, 3, 4],
    );
    this.tabs = [];
  ...
  select(index) {
    ...
    this.selected = index;
    this.panel.owner.rememberTab(`Shop.${this.side}`, index);
...
class ShopView {
  ...
  applyTransaction() {
    ...
    this.panel.owner.rememberTab("Shop.sell", this.tabs.sell.selected);
    this.tabs.sell.draw();
