...
/** Admission-time geometry includes every authored state/frame, not just normal/0. */
function buttonGeometry(panel, path, normal) {
  const body = { left: 0, top: 0, right: normal.width, bottom: normal.height };
  const focus = {
    left: Infinity,
    top: Infinity,
    right: -Infinity,
  ...
  const prefix = `${path}/`;
  let count = 0;
  for (const key in panel.assets) {
    if (!key.startsWith(prefix)) continue;
    const state =
      /^(normal|mouseOver|pressed|disabled|keyFocused)(?:\/\d+)?$/.exec(
        key.slice(prefix.length),
      );
    if (!state) continue;
    if (++count > 5 * 128) throw new Error("UI button frame budget exceeded");
    const asset = panel.assets[key];
    const bounds = state[1] === "keyFocused" ? focus : body;
    const left = normal.origin.x - asset.origin.x;
    const top = normal.origin.y - asset.origin.y;
    bounds.left = Math.min(bounds.left, left);
    bounds.top = Math.min(bounds.top, top);
    bounds.right = Math.max(bounds.right, left + asset.width);
    bounds.bottom = Math.max(bounds.bottom, top + asset.height);
  }
  return { body, focus: fitButtonFocus(body, focus, normal.origin) };
}

/** Browser geometry policy: preserve authored focus pixels, fit their envelope around the button. */
function fitButtonFocus(body, focus, origin) {
  if (!Number.isFinite(focus.left)) return null;
  const left = Math.min(focus.left, body.left - 1);
  const top = Math.min(focus.top, body.top - 1);
  const right = Math.max(focus.right, body.right + 1);
  const bottom = Math.max(focus.bottom, body.bottom + 1);
  const scaleX = (right - left) / (focus.right - focus.left);
  const scaleY = (bottom - top) / (focus.bottom - focus.top);
  return {
    x: left - (focus.left - origin.x) * scaleX,
    y: top - (focus.top - origin.y) * scaleY,
    scaleX,
    scaleY,
  };
}
...
class UIControl {
  constructor(panel, options) {
    ...
    this.geometry = buttonGeometry(panel, options.path, normal);
    for (const state of [
      "normal",
      "mouseOver",
      "pressed",
  ...
  position(x, y) {
    ...
    for (const name in this.states) {
      ...
      const focus = name === "keyFocused" && this.geometry.focus;
      if (focus) {
        sprite.container.scale.set(focus.scaleX, focus.scaleY);
        sprite.setPosition(x + focus.x, y + focus.y);
      } else {
        sprite.setPosition(x + this.origin.x, y + this.origin.y);
      }
    }
    this.element.style.left = `${x + this.geometry.body.left}px`;
    this.element.style.top = `${y + this.geometry.body.top}px`;
  }
  ...
  createElement() {
    ...
    const { left, top, right, bottom } = this.geometry.body;
    this.element.style.cssText = `position:absolute;width:${right - left}px;height:${bottom - top}px;`;
