  ...
  for (const [path, x, y, target] of HUD_BUTTONS) {
    panel.button(path, x, y, {
      label:
        target === "GameLogs"
          ? "Game Logs"
          : index.help[target]?.title || target,
      tooltip:
        target === "GameLogs"
          ? "Game Logs"
          : index.help[target]
...
function inventoryTabs(panel, branch, count, target = panel) {
  ...
  const select = (selected, notify = true) => {
    ...
    updateInventoryTab(target, branch, selected, notify);
  };
  ...
  const choices = branch === "Skill" ? target.skillTabChoices : [0, 1, 2, 3, 4];
  const saved = target.owner.restoreTab(branch, choices);
  select(branch === "Skill" ? choices.indexOf(saved) : saved, false);
}

function updateInventoryTab(target, branch, selected, notify) {
  ...
  if (notify) {
    target.owner.rememberTab(
      branch,
      branch === "Skill" ? target.skillTabChoices[selected] : selected,
    );
    target.owner.refreshProfilePanel(target);
  }
}

/** Shared native type1 Tab2 strip; all retained constructors use client y23. */
export function tabParts(panel, part, x, width) {
  const sprites = [];
...
export function showTabPart(sprites, state) {
  for (let i = 0; i < sprites.length; i++) {
    sprites[i].container.visible = i === state;
  }
}

export function tabLabel(panel, path, x, width) {
...
export function updateSkillTabs(panel, books) {
  ...
  panel.tabButtons = [];
  panel.selectedTab = 0;
  panel.skillTabChoices = books.slice(0, 5);
  panel.skillStart = 0;
