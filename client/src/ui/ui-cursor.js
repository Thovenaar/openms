...
export class UICursor {
  ...
  move(event) {
    if (this.owner.suspended) return;
    ...
    const inside = this.insideCanvas(event);
  ...
  drag(source, path) {
    ...
    const asset = source.assets[path];
    const resource = source.sources.get(path) || source.resource;
    this.ghost = new EntityAnimation(entity, resource.textures);
    // Native carry constructors offset the icon layer independently of cursor origin
    // (0083550d / 008d6409). Center the authored canvas, not its WZ anchor.
    this.ghost.setPosition(
      asset.origin.x - asset.width / 2,
      asset.origin.y - asset.height / 2,
    );
