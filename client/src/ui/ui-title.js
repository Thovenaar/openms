...
export function layoutTitle(panel, social) {
  ...
  state.tab = panel.owner.restoreTab("Title", [0, 1, 2, 3, 4]);
...
function drawTitle(state) {
  ...
  socialTabs(state, {
    ...
    select: (tab) => {
      state.tab = tab;
      state.panel.owner.rememberTab("Title", tab);
      state.position = 0;
      state.detailQuestId = null;
      state.selected = null;
      state.refresh();
...
function paintMedalRows(state, rows, body) {
  ...
  for (
    ...
    records.push({ entry, id, template: state.panel.owner.index.items[id], y });
  }
  replaceIcons(body, records, (layer, entry) => {
    paintMedalRow(state, layer, entry.entry, entry.y);
  ...
  const hit = body.hit(
    ...
    },
    {
      tooltip: () => ({
        ...
        source: { surface: body, path: template?.iconPath },
      }),
    },
  );
