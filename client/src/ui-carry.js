...
async function performItemDrop(owner, instance, target) {
  ...
  if (count > 1 && group !== 207 && group !== 233) {
    count = await owner.prompt({
      kind: "number",
      text: "How many would you like to drop?",
      value: count,
      min: 1,
      max: count,
      owner: actions,
...
export function dropCarriedItem(owner, event, element) {
  ...
  if (drag.pending || owner.hooks.isOperationPending?.()) return true;
  drag.pending = true;
  finishItemDrop(owner, drag, instance, target);
  return true;
}

async function finishItemDrop(owner, drag, instance, target) {
  try {
    const result = await performItemDrop(owner, instance, target);
    if (owner.bindingDrag !== drag) return;
    if (result.ok) {
      owner.endBindingDrag();
      owner.sound("DragEnd");
    ...
    drag.pending = false;
    if (owner.bindingDrag === drag && !carriedInstance(owner, drag)) {
      owner.endBindingDrag();
    }
  }
}
