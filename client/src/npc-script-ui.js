...
function placeInput(view, geometry) {
  ...
  view.input.style.top = `${geometry.y + Math.min(geometry.measured, geometry.body - 20) + 2}px`;
...
function placePlayer(view, geometry) {
  ...
  const bar = view.panel.assets["UtilDlgEx/bar"];
  const combined = bounds.height + bar.height;
  const barY =
...
function publishDestination(view, current) {
  ...
  if (!["shop", "closed", "blocked"].includes(current.kind)) {
...
function select(view, button) {
  ...
  for (const choice of view.panel.content.querySelectorAll(
    ...
    choice.style.textDecoration = selected ? "underline" : "none";
  }
  setPending(view);
  return true;
}
