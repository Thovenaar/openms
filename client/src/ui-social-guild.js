...
function drawGuildSummary(state, group, alliance, branch) {
  state.layer.image(`${branch}/guildinfo0`, 14, 45);
  socialText(
    state.layer,
    group?.name || (alliance ? "No Guild Union" : "No guild"),
    { x: 24, y: 49, width: 265 },
  );
  socialText(
    state.layer,
    group?.notice || "",
    { x: 18, y: 81, width: 266, height: 32 },
    { wrap: true },
  );
  socialList(
    state,
    alliance ? unionRows(state, group) : group?.members || [],
    { x: 14, y: 120, width: 274, height: 180 },
    {
      label: (row) =>
        row.groupHeader
          ? row.name
          : `${row.leader ? "Leader " : row.rank ? `${row.rank} ` : ""}${memberText(row)}`,
      tooltip: (row) =>
        row.groupHeader
          ? `${row.name}: ${row.count} members`
          : `${memberText(row)}\n${row.mapName}`,
      empty: group?.forming
        ? "Waiting for the founders to accept."
        : "No members.",
      activate: (row) =>
        row.groupHeader
          ? undefined
          : socialHook(state, "socialOpenUserInfo", row.id),
    },
  );
}
...
function unionRows(state, union) {
  if (!union) return [];
  const rows = [];
  ...
  for (const guild of union.guilds) {
    ...
    for (const member of guild.members) {
      ...
      if (unionMember) rows.push(unionMember);
    }
  }
  return rows;
}
