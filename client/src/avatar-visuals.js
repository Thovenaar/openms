...
import { resource } from "./stream-validation.js";
...
//00978c09 ->00766031,00bdafd8: unmodified melee rows for info/attack1..9.
const PREVIEW_MELEE = [
  [],
...
//00978d9c ->00766272,00bdb188: CashShop routes only bow/crossbow here.
const PREVIEW_RANGED = { 3: ["shoot1"], 4: ["shoot2"] };
...
/**00460aa0 validates actual weapon categories before selecting a170 cash weapon's family. */
function weaponFamily(id) {
  const family = Math.floor(id / 10000) % 100;
  return Math.floor(id / 1000000) === 1 &&
    ((family >= 30 && family <= 33) || (family >= 37 && family <= 49))
    ? family
    : 0;
}
...
function selectAppearance(profile, index) {
  ...
  return {
    ...
    weaponFamily: weaponFamily(weapon),
    ...
    stand: baseWeapon ? (baseWeapon.stand === 1 ? 1 : 2) : 1,
    walk: baseWeapon ? (baseWeapon.walk === 1 ? 1 : 2) : 1,
  };
}
...
function preparedPreview(animation, resource) {
  ...
  const attacks = (ranged ? PREVIEW_RANGED : PREVIEW_MELEE)[
