...
import { CASH_CURRENCIES, CASH_POLICY } from "./cash-commerce.js";
import { TRADE_SLOTS, tradeRechargeable } from "./local-trade-rules.js";
...
export class LocalSimulationControls {
  ...
  offerPrimaryInvitation(invitations) {
    ...
    this.run(async () => {
      const accepted = await this.owner.prompt({
        kind: "confirm",
        text: `${request.fromName}: ${request.kind}${request.groupName ? ` (${request.groupName})` : ""}. Accept this invitation?`,
      });
