...
import { mountNpcDialogue } from "./quest-ui.js";
...
export class NpcInteractions {
  ...
  get pending() {
    return this.loading || !!this.session?.pending || !!this.shop?.pending;
  }
  ...
  async open(record) {
    ...
    try {
      ...
      if (
        (this.owner.quests.byNpc.get(Number(record.templateId)) ?? []).length
      ) {
        this.record = {
          ...record,
      ...
      } else await this.runRoute(route);
      return true;
    } catch (error) {
  ...
  async startRoute() {
    if (this.pending || !this.isCurrent()) return false;
    this.loading = true;
    try {
  ...
  async runRoute(route) {
    ...
    this.session = new NpcScriptSession(
      route,
      this.store,
    ...
    if (result.view.kind !== "closed") await this.owner.ui.showNpc(this.record);
  }
  ...
  windowClosed(name) {
    if (!this.loading && (name === "Shop" || name === "UtilDlgEx")) {
      this.release();
    }
  }
