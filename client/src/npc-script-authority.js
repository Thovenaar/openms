...
function validateMarkupItemImage(token, environment) {
  const item = npcLookup(environment.items, Number(token.slice(2, -1)));
  requireNpc(
    item?.descriptor &&
...
function itemEffect(turn, node, args) {
  ...
  if (count >= 0) {
    requireNpc(
      inventoryType(id) !== 1 && !(id >= 5000000 && id <= 5000999),
      "NPC equipment/pet generation requires external instance authority",
      "npc-dependency",
    );
  }
  if (count > 0) grantItem(turn.profile, template, count);
