...
import { validateCharacterId } from "../profile/profile-validation.js";
import {
  validateNativeEvent,
  NATIVE_EVENT_LIMIT,
...
function validateScenarioOwners(value) {
  if (value.profile !== undefined) validateProfile(value.profile);
  if (value.characterId !== undefined) validateCharacterId(value.characterId);
  if (value.accountStorage !== undefined) {
    if (!value.profile) {
      throw new Error("Account replay requires its character baseline");
    }
    validateAccountStorage(value.accountStorage, value.profile);
  }
  if (value.peers !== undefined) {
    validateDiagnosticPeers(value.peers, value.characterId);
  }
}
...
export function validateScenarioRecording(value, buildId) {
  objectKeys(
    value,
    [
      "schemaVersion",
  ...
  const native =
    value.schemaVersion === 2 && value.mode === "native-fixed-tick";
  if (
    (!native && (value.schemaVersion !== 1 || value.mode !== "fixed-tick")) ||
  ...
  if (native) validateNativeBaseline(value.initialUI, spec);
  validateRecordedActions(value.actions, value.totalTicks, native);
  return { ...structuredClone(value), spec };
}

function validateNativeBaseline(initialUI, spec) {
  objectKeys(
    initialUI,
    ["windows", "positions", "viewport"],
    "native initial UI",
  );
  if (!Array.isArray(initialUI.windows) || initialUI.windows.length > 32) {
    throw new Error("Invalid native baseline windows");
  }
  for (const name of initialUI.windows) {
    validatePlayerCommand({ type: "ui", name });
  }
  if (!spec.characterId || !spec.accountStorage || !spec.peers) {
    throw new Error("Native recording requires its complete account baseline");
  }
  validateInitialPositions(initialUI.positions);
  validateNativeViewport(initialUI.viewport);
}

function validateNativeViewport(viewport) {
  objectKeys(viewport, ["width", "height"], "native viewport");
  if (
    ...
    throw new Error("Native viewport exceeds the logical renderer bounds");
  }
}

function validateRecordedActions(actions, totalTicks, native) {
  const maximum = native ? NATIVE_EVENT_LIMIT : MAX_ACTIONS;
  if (!Array.isArray(actions) || actions.length > maximum) {
    throw new RangeError(`Recording supports at most ${maximum} actions.`);
  }
  let previousTick = 0;
  for (const entry of actions) {
    const nativeEvent = native && Object.hasOwn(entry, "native");
    objectKeys(
      entry,
      nativeEvent
        ? ["tick", "native", "accepted"]
    ...
    tickCount(entry.tick, totalTicks);
    if (entry.tick < previousTick) {
      throw new Error("Recorded tick offsets must be ordered.");
    }
    if (nativeEvent) validateNativeEvent(entry.native);
    else validatePlayerCommand(entry.command);
    previousTick = entry.tick;
  }
}

/** Initial window coordinates are configuration, never imported DOM or live state owners. */
function validateInitialPositions(positions) {
  if (!Array.isArray(positions) || positions.length > 256) {
    throw new Error("Invalid native baseline positions");
  }
  for (const position of positions) {
    objectKeys(position, ["name", "x", "y"], "native window position");
    if (typeof position.name !== "string" || position.name.length > 64) {
      throw new Error("Invalid native position name");
    }
    if (
      ![position.x, position.y].every(
        (value) => Number.isFinite(value) && Math.abs(value) <= 16384,
      )
    ) {
      throw new Error("Invalid native position coordinates");
    }
  }
}
...
export class AgentScenarios {
  ...
  async _enter(spec, generation) {
    ...
    try {
      this._randomState = spec.seed;
      this._gameplayRandomState = spec.seed;
  ...
  async _exit() {
    if (this._spec) this.hooks.clearInput();
  ...
  async _step(ticks, generation) {
    ...
    for (let transitions = 0; transitions <= 32; transitions++) {
      ...
      if (this.hooks.waitIdle) await this.hooks.waitIdle();
  ...
  async run(value) {
    this._idle();
    const recording = validateScenarioRecording(
      value,
      buildIdentity(this.hooks),
    );
    const generation = this._lease();
    this._busy = true;
    ...
    try {
      ...
      this._lease(generation);
      if (recording.mode === "native-fixed-tick") {
        await this.hooks.prepareNative(recording.initialUI);
      }
  ...
  async _play(recording, generation) {
    ...
    while (
      ...
      const result = action.native
        ? await this.hooks.dispatchNative(action.native)
        : await this.hooks.dispatch(structuredClone(action.command));
