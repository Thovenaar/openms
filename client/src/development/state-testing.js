...
const SECTIONS = [
  ["quests", "Quests"], ["connection", "Connection"],
  ["content", "Content identity"], ["prediction", "Input/prediction"],
  ["operations", "Operations/events"], ["peers", "Peers/session"],
];
const CONNECTION_FIELDS = ["status", "serverTick", "inputSeq", "lastEventSeq",
  "pendingOperations", "queuedMessages", "queuedBytes", "bufferedBytes"];

/** Inspection serialization is bounded and never changes the observed model. */
export function inspectionText(value) {
  if (value === undefined || value === null) return "Not available in this session.";
...
function addAction(parent, label, action, state) {
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      const result = await state.command(typeof action === "function" ? action() : action);
...
function questAction(row, kind) {
  return { kind, questId: row.entry.id, conversationId: row.offer?.conversationId,
    step: row.offer?.step };
}
...
function createQuestRow(parent, state) {
  ...
  row.abandon = addAction(actions, "Abandon…", () =>
    questAction(row, "inspection.quest-abandon"), state);
  row.accept = addAction(actions, "Accept offered quest", () =>
    questAction(row, "inspection.quest-accept"), state);
  row.claim = addAction(actions, "Claim offered reward", () =>
    questAction(row, "inspection.quest-claim"), state);
  return row;
}
...
function objectiveText(objectives) {
  ...
  const rows = [];
  for (let index = 0; index < Math.min(objectives.length, MAX_OBJECTIVES); index++) {
    rows.push(objectiveLine(objectives[index]));
  }
  if (objectives.length > MAX_OBJECTIVES) rows.push("Further objectives: open native quest journal.");
...
function updateQuestRow(row, entry, offer) {
  ...
  row.abandon.hidden = ![1, "active"].includes(entry.state) || entry.giveUp === false;
...
function observedQuestMatches(value, query) {
  ...
  return entries.filter((entry) => `${entry.name ?? ""} ${entry.id}`.toLowerCase().includes(query));
}

function questSearchSummary(matched, mode) {
  if (matched > MAX_QUEST_ROWS) return `${matched} matches; first ${MAX_QUEST_ROWS} shown. Refine the search.`;
  const action = mode === "online" ? "Accept/claim require a current server NPC offer." : "Open the native journal for quest actions.";
  return `${matched} observed quests. ${action}`;
}

function mountQuests(root, state) {
  addAction(root, "Open native Quest journal", { kind: "inspection.quest-journal" }, state);
  ...
  function refresh(next = value) {
    value = next;
    const matches = observedQuestMatches(value, filter.value.trim().toLowerCase());
    const visible = new Set();
    for (const entry of matches.slice(0, MAX_QUEST_ROWS)) {
      visible.add(entry.id);
      let row = rows.get(entry.id);
      if (!row) { row = createQuestRow(list, state); rows.set(entry.id, row); }
      updateQuestRow(row, entry, value.offer);
      if (state.mode === "offline") row.abandon.hidden = true;
    }
    for (const [id, row] of rows) {
      if (!visible.has(id)) { row.root.remove(); rows.delete(id); }
    }
...
function mountConnection(root) {
  ...
  for (const key of CONNECTION_FIELDS) {
    inspectionElement(list, "dt", key.replace(/[A-Z]/g, (letter) => ` ${letter.toLowerCase()}`));
    fields.set(key, inspectionElement(list, "dd"));
  }
  return (value) => {
    for (const [key, output] of fields) output.textContent = String(value?.[key] ?? "Not applicable");
  };
}
...
export function mountStateTesting({ root, mode, read, command, signal }) {
  if (!["online", "offline"].includes(mode)) throw new Error("Unknown authority mode");
  const panel = inspectionElement(root, "section");
  panel.className = "state-testing";
  inspectionElement(panel, "p", mode === "online"
  ...
  if (mode === "online" && command) {
    ...
    actions.className = "state-testing-actions";
    addAction(actions, "Request authoritative resync", { kind: "inspection.resync" }, state);
    addAction(actions, "Reconnect session", { kind: "inspection.reconnect" }, state);
  }
  function refresh() {
    const value = read();
