...
export class GameplayNoticeLog {
  ...
  publish(event) {
    ...
    rasterRow(row);
    // 0089b159 calls0089b60f after EVERY admission: row layout snaps, unlike quickslot motion.
  ...
  syncDensity() {
    ...
    if (x === this.densityX && y === this.densityY) return;
    const width = Math.ceil(ROW_WIDTH * x);
  ...
  draw() {
    this.context.clearRect(0, 0, ROW_WIDTH, SURFACE_HEIGHT);
    for (let index = 0; index < this.count; index++) {
      const row = this.rows[(this.first + index) % ROW_COUNT];
      if (row.alpha === 0) continue;
      this.context.globalAlpha = row.alpha / 255;
      this.context.drawImage(row.canvas, 0, row.y, ROW_WIDTH, ROW_HEIGHT);
    }
...
function rasterRow(row) {
  const context = row.context;
  context.clearRect(0, 0, ROW_WIDTH, ROW_HEIGHT);
  if (!row.text) return;
  const x = ROW_WIDTH - Math.ceil(context.measureText(row.text).width);
...
function itemText(log, event) {
  ...
  return plural
    ? `You have gained items in the ${tab} tab (${name} ${event.amount})` //0x153d
    : `You have gained an item in the ${tab} tab (${name})`; //0x153e
}
