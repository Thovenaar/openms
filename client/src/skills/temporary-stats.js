import { itemConditionsMatch } from "../items/item-conditions.js";
...
const DEFENSE_ATT_INDEX = TEMPORARY_STATS.indexOf("defenseAtt");
// Cosmic BuffStat: preserve original low64 field indices; append upper64 states.
...
const MASK_UPPER_HIGH = new Array(13).fill(0);
for (const [low, high, upperHigh] of EXTRA_MASKS) {
  MASK_LOW.push(low);
  MASK_HIGH.push(high);
  MASK_UPPER_HIGH.push(upperHigh);
}
...
export function configureTemporaryState(
  ...
  for (let index = 0; index < TEMPORARY_STATS.length; index++) {
    ...
    // Cosmic packet arbitration uses4; the original WZ effect parameters stay separate.
    const value =
      (field === "mesoupbyitem" ||
        field === "itemupbyitem" ||
        field === "defenseAtt") &&
...
export class TemporaryStats {
  constructor() {
    ...
    this.reservedSources = new Int32Array(MAX_TEMPORARY_STATS);
    this.reservedOwners = new Int32Array(MAX_TEMPORARY_STATS);
    ...
    this.derived.defenseAttElement = 0;
    this.derived.defenseAttPercent = 0;
  }
  ...
  /** Optional second source accounts for an admitted controller's future publication. */
  canStart(source, additionalSource = null) {
    const reserved = this.reservedIndex(source) >= 0;
    if (
      this.destroyed ||
      (this.reservation && (!reserved || additionalSource !== null))
    ) {
      return false;
    }
    let occupied = this.count + this.reservedCapacity;
    if (!reserved && !this.find(source)) occupied++;
    if (
      additionalSource !== null &&
      additionalSource !== source &&
      this.reservedIndex(additionalSource) < 0 &&
      !this.find(additionalSource)
    ) {
      occupied++;
    }
    return occupied <= MAX_TEMPORARY_STATS;
  }

  reservedIndex(source) {
    for (let index = 0; index < this.reservedCount; index++) {
      if (this.reservedSources[index] === source) return index;
    }
    return -1;
  }

  /** Persistent capacity only: no source, values, timer, or visible buff is fabricated. */
  reserveSource(source, owner) {
    const index = this.reservedIndex(source);
    if (index >= 0 && this.reservedOwners[index] === owner) return;
    if (index >= 0 || !this.canStart(source)) {
      throw new Error("Unadmitted temporary source capacity reservation");
    }
    this.reservedSources[this.reservedCount] = source;
    this.reservedOwners[this.reservedCount++] = owner;
    if (!this.find(source)) this.reservedCapacity++;
  }

  releaseOwner(owner) {
    for (let index = this.reservedCount - 1; index >= 0; index--) {
      if (this.reservedOwners[index] !== owner) continue;
      if (!this.find(this.reservedSources[index])) this.reservedCapacity--;
      for (let next = index + 1; next < this.reservedCount; next++) {
        this.reservedSources[next - 1] = this.reservedSources[next];
        this.reservedOwners[next - 1] = this.reservedOwners[next];
      }
      this.reservedCount--;
    }
  }
  ...
  start(state) {
    ...
    this.count++;
    if (this.reservedIndex(state.source) >= 0) this.reservedCapacity--;
  ...
  recompute() {
    ...
    for (let index = 0; index < this.count; index++) {
      ...
      state.previousMaskUpperHigh = state.maskUpperHigh;
      state.maskLow = 0;
      state.maskHigh = 0;
      state.maskUpperHigh = 0;
      if (
        state.unavailable ||
        !itemConditionsMatch(state.conditions, this.context)
      ) {
  ...
  project() {
    for (let stat = 0; stat < TEMPORARY_STATS.length; stat++) {
      ...
      winner.maskUpperHigh |= MASK_UPPER_HIGH[stat];
    }
    this.projectDefenseAtt();
    ...
    for (let index = this.visibleCount - 1; index >= 0; index--) {
      ...
      if (!(state.maskLow | state.maskHigh | state.maskUpperHigh)) {
    ...
    for (let index = this.count - 1; index >= 0; index--) {
      ...
      const gained =
        ...
        (state.maskUpperHigh & ~state.previousMaskUpperHigh);
      if (!gained) continue;
      this.removeVisible(state);
  ...
  projectDefenseAtt() {
    const defense = this.winners[DEFENSE_ATT_INDEX]?.itemValues;
    this.derived.defenseAttElement = defense?.defenseAtt ?? 0;
    this.derived.defenseAttPercent = defense?.prob ?? 0;
  }
