 ...
 * prompt receives {kind:'number'|'confirm'|'notice',operation,text,min,max,defaultValue,itemId,signal}.
...
export class NpcShop {
  ...
  /** A refusal is a cancellable native modal, never a durable transaction lease. */
  async rejectOperation(error) {
    if (error.code !== "cancelled") callHook(this.context, "error", error);
    if (error.code !== "insufficient-mesos") return failure(error);
    try {
      ...
      if (controller.signal.aborted || !result?.ok) {
        return failure(
          profileError("cancelled", "The shop operation was cancelled."),
        );
      }
      this.admit(this.store.profile);
      return failure(error);
    } catch (noticeError) {
      callHook(this.context, "error", noticeError);
      return failure(noticeError);
    }
  }
  ...
  async execute(kind, payload) {
    ...
    try {
      ...
      outcome = await this.rejectOperation(error);
    } finally {
      this.promptController = null;
      this.phase = "idle";
      this.notify();
