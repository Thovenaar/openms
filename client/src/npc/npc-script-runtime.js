...
function publication(sessionId, revision, turn) {
  ...
  return {
    state: Object.freeze({
      ...
      craftingScroll: turn.view.kind === "closed" ? false : turn.craftingScroll,
      savedMapIds: Object.freeze(
        [...turn.context.dependencies.mapIds].filter(
...
function travelEffect(turn) {
  return turn.effects.find((effect) => effect.kind === "warp") ?? null;
}

function applyPreparedTravel(prepared, expected, turn) {
  const actual = travelEffect(turn);
  requireNpc(
    actual?.mapId === expected?.mapId &&
      actual?.portal === expected?.portal &&
      actual?.randomSpawn === expected?.randomSpawn,
    "NPC destination changed while the field was loading",
    "npc-travel",
  );
  if (!actual) return;
  requireNpc(
    prepared?.isCurrent(),
    "Prepared NPC field is stale",
    "npc-travel",
  );
  prepared.apply(turn.profile);
...
export class NpcScriptSession {
  ...
  async #commitTurn(turn, request) {
    const destination = travelEffect(turn);
    let prepared = null;
    try {
      if (destination) {
        prepared = await this.#environment.prepareTravel({
          mapId: destination.mapId,
          ...(destination.randomSpawn
            ? { randomSpawn: true }
            : { portal: destination.portal }),
        });
        requireNpc(
          prepared &&
            ["isCurrent", "apply", "publish", "release"].every(
              (name) => typeof prepared[name] === "function",
            ),
          "NPC field preparation returned an invalid lease",
          "npc-travel",
        );
      }
      ...
      await this.#store.commitProfile((draft) => {
        ...
        applyPreparedTravel(prepared, destination, turn);
        ready = publication(this.#sessionId, this.#view.revision + 1, turn);
      });
      // Prepared publication is a synchronous no-fail swap, never another load/save.
      prepared?.publish();
      return ready;
    } finally {
      prepared?.release();
  ...
  async #turn(input) {
    ...
    try {
      ...
      const turn = this.#prepare(structuredClone(this.#store.profile), request);
      let ready = publication(this.#sessionId, this.#view.revision + 1, turn);
      const committed = turn.effects.length > 0;
      if (committed) {
        ready = await this.#commitTurn(turn, request);
      }
  ...
  close() {
    try {
      ...
      if (!this.#closed) {
        ...
        this.#closed = true;
        this.#state = Object.freeze({ ...this.#state, craftingScroll: false });
      }
      return {
        ok: true,
        view: this.#view,
