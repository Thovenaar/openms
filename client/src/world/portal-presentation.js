...
export function portalRectangleContains(portal, pose, halfWidth, halfHeight) {
  const x = Math.trunc(pose.x), y = Math.trunc(pose.y);
  return x >= portal.x - halfWidth && x < portal.x + halfWidth &&
...
export function updatePortalGraphics(scene, record, desired) {
  if (!record.entityId || (record.portal.type !== 10 && record.portal.type !== 11)) return;
  ...
  if (record.phase === "portalStart" && animation.elapsedMs >= animation.current.duration) {
    record.phase = "portalContinue";
    animation.setAction(record.phase);
  } else if (record.phase === "portalExit" && animation.elapsedMs >= animation.current.duration) {
    record.phase = "hidden";
    animation.container.visible = false;
  }
}
