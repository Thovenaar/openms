...
Ordinary mob damage, hit pose and recoil have separate admission. Original `pushed` (absent default1) gates ordinary hit reaction; subthreshold damage still changes HP. Existing attack pose/pending impact survive admitted recoil, and repeated hits do not restart or reverse a running recoil. Ordinary130-pixels/s recoil uses400-pixels/s² braking; recovered weapon chance selects the separate strong300/200 branch, not ordinary reaction eligibility. Connected-floor motion is not clipped to patrol `rx0/rx1`; flying/special controllers remain unsupported. Player-hit mobs do not inherit player protection blink: native mob blink requires `damagedByMob`. Player protection separately uses1500ms and two-of-four30-ms updates per gray/normal RGB cycle, not disappearance. See the [corrected combat contract](offline-combat.md) and [native Hector/Red Snail evidence](ingame-validation/expanded/gameplay/report.json).
...
The [expanded native gameplay report](ingame-validation/expanded/gameplay/report.json) proves Red Snail32/hit1, Hector798 versus800 recoil admission, retained attack1 pose and keyboard Talk to Maya. The [earlier interaction report](ingame-validation/interaction-corrections/report.json) retains Blue Snail loot, Cody scrolling, Maya's requirement refusal and Nella's once-only1000-meso quest transaction. These are scoped examples, not all-template/all-quest coverage.
...
`client/test/life-interaction.test.js` covers dc edges/facing, exact target identity, release routing, inspection separation, stale/hidden/dead/modal admission and presentation-gate lifecycle using production LifeSystem/Pixi objects. Main's consolidated147-test gate passes; native scenarios are indexed in the [expanded acceptance](validation.md#current-expanded-fidelity-acceptance).
