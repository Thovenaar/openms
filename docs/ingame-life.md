# In-game life: original metadata preview

This implementation is an **offline, non-authoritative metadata preview**, not a reconstruction of live spawn activation. It uses only the supplied `Maplestory-Client` WZ archives and original executable. No C/C++ source, original runtime recording, Windows runtime, or server was available. Names/resource artwork can be inspected; AI, combat, HP, dialogue, shops, death state, drops, and respawn are not synthesized.
...
`scene` supplies the original shared scene contract plus `scene.overlays`, a persistent world-space overlay container reattached after region refresh. Main advances `EntityAnimation` before `LifeSystem.update(ms)`. Call `life.refresh()` after `scene.byId` membership changes, outside the render loop; constructor calls it once to initialize paused first draw. Destroy the life owner **before** scene/entity/texture ownership. The owner never loads, decodes, uploads or releases world artwork itself. World artwork remains demand-driven through package-v2 regions, shared pixel hashes, AtlasStore budgets and leases.

Additional public preview methods:

- `select(id)` selects a stable authored `life:<index>` placement.
- `setPreviewAction(id, action)` explicitly selects an original artwork branch. Selecting `die1` shows a looping resource preview, not death or respawn. No action triggers, eligibility, or transition permission are inferred from branch names.
- `interact(id)` forwards only NPCs through the server-unavailable `hooks.onInteract` boundary. Mob clicks instead open/select the local metadata panel and nondamaging geometry.
...
Interaction records expose `{id, templateId, name, functionName, authority, kind, authored, info, interactionGeometryKnown, mode}`. `authority` is `server-unavailable; metadata-preview`. NPC records enter the UI's server-unavailable dialog boundary; mob records are nondamaging inspection. No original sound is dispatched from mere metadata selection. Hook promises attach error handling. The preview controls stop bubbling keyboard events; the shared input/UI owner must clear held gameplay input when DOM controls take focus and ignore editable/control targets.
...
Limits are engineering bounds, not original constants: 4,096 placements per map, 512 templates per extraction context, 128 action branches, 1,024 authored frames per action, 32,768 metadata nodes per source IMG, depth 64, 32 mob link hops. Unsupported/cyclic input throws rather than truncating. The acceptance set contains no IMG links; UOL frame aliases are present and resolved with the existing parser.
...
| `006d5c9a`              | In-game NPC nameplate consumer suppresses both name and optional nonempty function when template `hideName` at `+0x18` is nonzero. Browser typography, outline, spacing, and `[metadata preview]` label remain explicitly browser-owned, not original font parity.                                                                                                                                            |
...
Retained decoded string identities are corroborated by `docs/ghidra-client/decoded-strings.txt`. Prior body/attack research remains in `docs/hitboxes.md` and `docs/ghidra-physics-hitboxes/`; outgoing/incoming combat is deliberately not implemented here.
...
All placements initially preview `stand`; authoring flags do not authorize original spawn activation. Placement `hide=1` suppresses the preview unless the user explicitly reveals it. Original NPC template `hideName=1` continues to suppress its name/function even under reveal. Limited/event records are labeled previews, not gated by invented dates or scripts.
...
- Swept mob body uses the current frame body at both previous/current actor positions. Static metadata previews normally have coincident current/swept geometry. No autonomous movement is added to create a sweep.
- NPC `dcLeft/dcTop/dcRight/dcBottom` are interaction rectangles, **never physical body or damage geometry**. Missing or partial fields stay unsupported; zero-size authored rectangles stay zero. Artwork clicking without a complete dc rectangle is an explicitly non-authoritative inspection affordance, not an inferred interaction range.
- Authored `fh` is resolved against the existing simulation's prepared segment map; selected geometry shows that exact segment, the original `x/y → x/cy` contact line and `rx0..rx1` at `cy`. It does not attach or integrate a new physics actor.
...
No original visual parity, live-spawn fidelity, damage authority, or server-compatible interaction protocol is claimed.
