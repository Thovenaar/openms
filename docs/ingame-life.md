...
Ordinary mob hit reaction now distinguishes damage from reaction eligibility: original `pushed` (absent default1) gates available `hit1`, attack interruption and grounded knockback. Subthreshold damage still changes HP without those reactions. Authored `hit1` duration and recovered130-pixels/s initial velocity with400-pixels/s² midpoint braking replace the old fixed shove; supported connected-floor motion is not clipped to patrol `rx0/rx1`. This does not implement flying/special controllers. Ordinary player-hit mobs do not inherit player invulnerability blink: the original mob blink branch requires `damagedByMob`. Player positive protection separately uses1500ms and two-of-four30-ms updates per gray/normal RGB cycle (**60ms per phase**), not disappearance. [Exact interaction evidence](ghidra-client-corrections/interaction-skill-combat-provenance.json) supplies the distinction; the reported fast player appearance remains an unresolved original-runtime comparison.
...
The [current browser report](ingame-validation/interaction-corrections/report.json) separately proves native Blue Snail kills and original item/meso pickup, plus a Red Snail's HP40→8 ordinary `hit1` reaction. NPC input covers Cody's scrolling list, Maya's blocked requirement, and Nella's original next/back/decline/accept pages with a1000-meso transaction committed once. These examples are not an all-template or all-quest runtime claim.
...
`client/test/life-interaction.test.js` adds focused dc-edge/facing, exact target identity, release routing, inspection separation and stale/hidden/dead/modal admission regressions using production LifeSystem routing and Pixi's actual hit picker. They were intentionally **not run by this slice**; Main owns consolidated validation and native-browser verification.
