...
**Aim caveat.** Drawing remote actors in the past means the player also aims at their past position. Mob hit validation currently tests the mob at the server's present tick, so a moving mob can leave a melee rectangle during the extra playout window. Server-side lag compensation — rewinding a mob body to the tick the attacker was rendering — is the natural companion to this change and is **not implemented**. It is the next required step before this presentation policy is complete for combat.
