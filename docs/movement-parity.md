...
[RemoteMotion](../client/src/online/remote-motion.js) draws mobs and other remote entities continuously from the latest server position and velocity. New observations blend their positional error over 180 ms instead of snapping the animation at packet arrival. Prediction is bounded to 600 ms without an update, braking over the final 150 ms; grounded travel clamps to the current foothold and follows its slope. Death stops extrapolation. A changed mob generation, explicit relocation or discrepancy over 256 px resets presentation immediately. These are browser presentation policies, not recovered AI rules.

Remote animation clocks advance between publications and seek only on a changed action/start tick or mob generation. No predicted position or action is written back into server entity state. The [combat latency check](validation-method.md#combat-latency-check) combines native delayed-network input with deterministic motion/correction tests; it does not establish all original skill or AI-controller parity.
