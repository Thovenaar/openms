...
[RemoteMotion](../client/src/online/remote-motion.js) draws remote actors continuously from received position and velocity. New observations blend their positional error over **180 ms**. Prediction ends after **600 ms** without an update, braking over the final **150 ms**. A changed mob generation or discrepancy over 256 px resets presentation immediately. An explicit relocation holds its destination until a full movement state arrives; old velocity and foothold contact cannot pull the actor back. These are browser presentation policies, not recovered AI rules.
...
Remote animation clocks advance between publications. Delayed copies of the same attack retain the furthest presented phase; a new action/start tick resets it. The field-owned visual clock stops during pause/inactive presentation, including confirmed pickup arcs. No predicted position or action is written back into server entity state. Drops use their separate [known flight and hover plan](drop-motion.md#online-presentation-through-delayed-updates), which can continue beyond the actor prediction horizon.
