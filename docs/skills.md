...
### Preset job admission audit

`bun server/tools/check-skill-admission.js [--generated-root DIR] [--jobs ID,ID] [--map ID] [--report FILE] [--strict]` proves job-wide admission instead of replaying one skill at a time. For every packaged preset job it stages the development profile through `stageJobPreset`/`applyJobPresetLoadout`, prepares the real server combat and skill runtime from the packaged catalog, lands the actor with production physics, then evaluates `SkillSystem.castError` — requirements, flags, field/state gates, prepared resources, authored actions, controller admission and costs — for **every** skill in that job's ancestor books. Each skill records its learned rank, staged weapon type, first underlying refusal string, and a deterministic classification: `ok`, `by-design` (disabled, timed, GM/book, passive, unsupported-with-reason, weapon-restricted, or a situational map/state/party/resource gate), or `gap`. Unknown refusal strings fail closed as gaps, and casts that throw are listed separately.
