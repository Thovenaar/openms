...
Current consolidated source validation passes strict lint and **147 tests /876 assertions**. The all-record report uses the original catalog and actual default-map actor manifest. A scoped production probe exercised Recovery's duration/expiration boundary, mutation-free cooldown refusal, Magic Guard shortfall/death, Stance expiry and Haste modifiers without visual/audio decoding. The subsequent [native gameplay report](ingame-validation/expanded/gameplay/report.json) separately proves Recovery, Nimble Feet, Stance and Magic Guard through real bindings, with exact observed timing and scoped audio capture. The [tooltip replay](ingame-validation/expanded/items/evidence.json) proves live rank1→2 descriptions after correcting dictionary lookup. These examples do not exercise every supported skill or unlock unavailable controllers.
