...
Field preparation keeps the original mushroom in a Windows 95 window with a progress bar, file/byte counters, destination name and current asset category. Fullscreen loading is restricted to map preparation that actually starts an uncached asset download. Cached map entry and same-map character changes never admit it. Once needed, it lasts through that map's preparation, including decode/upload; cancellation releases its owner. The counters describe discovered dependencies, not an invented total for the whole world. Other network/decode work uses a 310px-wide Windows 95 download button anchored in the viewport's bottom-right corner. Its gray beveled frame, navy spinner, counters and progress bar open a native modal with current files, region queue, cache usage and Pause/Resume. Only that button intercepts pointer input; the modal owns keyboard focus while open. Reduced-motion preferences stop rotation. The ordinary between-map brightness fade remains separate.
