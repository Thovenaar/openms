...
- The console is one sidebar with a **Tool section** dropdown (`#console-section`), not a row of tabs. Its six sections — **Field**, **Character**, **World**, **Diagnostics**, **Agent** and **Settings** — are task groups, not a sequence. Selecting an option reveals exactly one panel; the others carry `hidden`. Their DOM remains mounted, so switching sections preserves unsaved form values. The chosen section persists under `maple-inspection-section-v1`; the offline shell defaults to Field and opens on desktop.
- **Field** keeps current-field readout, map travel, monster spawn and the toolbar pause/reload/step controls together. Offline, map travel and spawn use the validated local field. Online, they are audited server development requests that require an authorized GM developer session, and the server validates field ownership. The redundant Movement keys disclosure is removed; native KeyConfig remains in the toolbar. Volume readouts show percentages without changing the native 0–128 audio range.
...
- **Diagnostics** groups runtime diagnostics (stepping, physics status, settings and status output), the error log and the state/testing host. **Diagnostics → Error log** projects the same bounded, deduplicated session journal as native Game Logs into selectable readonly text. Errors never insert a banner above Field or shift its controls. New records preserve text while it is focused for copying; blur refreshes the journal. Successful map changes clear current failure state, not log history. Startup failures remain visible before the renderer is ready. Online, resync and reconnect use the live transport, while pause and step remain audited server development requests.
...
5. At390px width and with keyboard-only Tab/Enter navigation, reach theme selection, all six sections, preset Apply/Discard, map selection, pause and diagnostics. Confirm active-section state, Agent-only permissions and the transport toolbar above the sections. Scroll the console without losing access to the game.
