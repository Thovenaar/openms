...
- **World** owns the current-map readout, map selection/travel and monster selection/spawn alongside scene inspection. Offline, map travel and spawn use the validated local field. Online, they are audited server development requests requiring an authorized GM developer session, and the server validates field ownership. Pause/reload and native KeyConfig remain in the shared toolbar outside the section panels; Step is under **Diagnostics → Runtime diagnostics**. Volume readouts show percentages without changing the native0–128 audio range.
...
- Pause/resume, key bindings and reload remain in the shared toolbar **above all five sections**, outside panel scrolling. Step and runtime status are in Diagnostics, alongside the error log. Agent permissions/Exit stay in Agent. Static console navigation initializes before renderer startup, so a renderer failure cannot strand the offline controls.
...
1. Choose a preset and press **Stage preset**.
2. Review the staged values, equipment, ammunition, skills and bindings. The live profile has not changed.
3. Press **Apply staged preset** beside the preview, or **Apply changes** for the complete draft. Both use the same `onProfileEdit` hook: the validated local profile transaction offline and the audited development endpoint online. All normal schema, skill catalog, ownership and persistence checks remain in force.
...
Job presets use the packaged skill books and the [validated loadout rules](offline-profile.md#development-job-loadouts). Switching jobs preserves prepared summon records during weapon replacement instead of treating them as player attacks. An unchanged utility preset reports **The character already matches these values** without sending an empty server edit. The [fresh recovery notes](original-resource-audit.md#character-presets-and-development-authority) retain the regression and two-player evidence.
...
**Save checkpoint** and **Destructive actions → Reset** are offline controls. Online uses server persistence and does not accept a browser-local reset or imported save as a GM operation. **Revive character** retains its existing admission rules. Reactor offering uses an owned inventory item and the existing offering hook, not a fabricated inventory grant.
...
Online GM pause, step, monster spawn and physics controls permit ordinary players on the same field. The authority still checks development mode, developer role, field ownership and busy/retiring participants; players do not gain GM permissions. Pause first before stepping or editing physics. The shared field update reaches every current participant. Unknown development outcomes recover the same operation ID; the UI must not create a second mutation to guess whether the first committed.
