...
| `POST /api/v1/characters`   | `{csrfToken,name,gender,skin,face,hair,str,dex,int,luk,weapon?,top?,bottom?,overall?,shoes?,hat?}` | Creates one account-owned character and returns its summary: name `4..13` letters/digits unique on the account (`NAME_TAKEN`), each stat an integer `4..13` summing to 25 (the packaged baseline total), and skin/face/hair ids that exist as packaged catalog cosmetics, so the endpoint can never grant unpainted art. Optional starter gear ids must be packaged non-cash equipment of the field's authored slot group at level 10 or below; the equip slot is derived server-side from `equippedSlots[0]`, an omitted field equips nothing in that slot, and `overall` conflicts with `top`/`bottom`. Bounded per account (`CHARACTER_LIMIT`). Level 1, job 0, EXP 0, mesos 0 with vitals recomputed, inserted atomically. |
