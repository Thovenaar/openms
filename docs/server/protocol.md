...
Register opens a separate Windows95-themed account popup; registration still provisions one beginner atomically. Selecting a character enters the world directly, without a world or channel list. An account with no characters opens name entry; the next phase previews the recovered appearance choices. Creation submits the packaged baseline `STR12/DEX5/INT4/LUK4` (total25), not an invented dice roll. Launcher-created development accounts remain separate from registration. Names are unique per account; the name phase checks that account's current roster and final creation repeats the authoritative check under the account lock. Global name uniqueness is not part of this contract.
...
6. Heartbeats detect a dead connection; session revocation is checked on every admission and expiry is scheduled server-side. On transient disconnect, inputs become neutral after the bounded hold window; the avatar remains in simulation during the30-second reconnect grace, not instantly safe or removed from combat. Explicit logout is different: session revocation closes admissions and removes the actor from simulation immediately, then fences pending durable work before checkpoint and writer-lease release.
