...
## Fresh resource, preset and chat follow-up

[The original resource audit](../original-resource-audit.md) inventories every IMG in the16original resource archives, including property-name/type/path evidence independent of extraction. It records the534packaged skills' controller classifications and missing item records without treating those declarations as full gameplay proof.

The same pass fixes post-commit Hero/Corsair preset replacement when a previously prepared summon has no player-weapon specification, adds Apply beside the staged preset preview, and handles unchanged utility presets without an empty edit. Shared GM field controls admit ordinary-player presence while preserving developer/development-server authorization. Destroyed logout skill resources no longer crash another player's publication.

Online map and social chat now share channel metadata, sender echoes and message-ID deduplication. The expanded log uses recovered whisper/buddy/party/guild/alliance colors. Map messages reach the whole actual field instance, including players outside visual interest range. Private-channel membership, preferences and blacklist admission remain server-owned; JSONB key ordering does not invalidate an otherwise matching group. Received history is session-local, not a retrospective server archive. [Native two-player proof](../native-ui-validation/drop-chat-presets/report.json) covers the repaired preset/chat/drop/GM paths and reconnects; the broader inventory still has the explicit gaps above.
