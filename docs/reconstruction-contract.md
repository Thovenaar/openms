...
Atomic portal replacement validates the packaged destination and exactly one named target, enters at `(x,y-10)`, retains the old scene on failure, and invalidates stale callbacks. While replacement is pending, the retained scene is visible but gameplay authority is paused. Offline routing is not server authorization. Nearby eligible NPC clicks enter the data-backed quest owner; missing scripts are explicit. Mob inspector clicks remain nondamaging, separate from fixed-tick local combat. Audio/effect inspection cannot mutate character state; committed local progression can request the original LevelUp effect.
