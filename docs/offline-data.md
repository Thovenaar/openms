...
Cosmic is an authorized **server reference**, not original Nexon client source or a replacement for original WZ metadata. This inventory was executed against `/Users/k/Development/tensorfish/Cosmic`. No SQL, Cosmic JavaScript, Java server, Windows client, or full asset extraction was executed by the converter.
...
- `sql-data.js` is the shared bounded reader. `drop-data.js` now uses it rather than scanning arbitrary six-number tuples. Drop extraction keeps its existing `mobs`, `itemIds`, `provenance`, and coverage contract and uses the Cosmic default root.
- `convertServerData({serverRoot?})` returns `{report,datasets}` without publishing. `extractServerData({output,serverRoot?})` publishes through the existing atomic, content-addressed `resource()` helper and returns `{schemaVersion:1,authority,datasets,report,summary}`. `datasets` maps `shops`, `drops`, `crafting`, `cards`, and `cash` to ordinary `{url,sha256,bytes}` resource descriptors. `report` is another such descriptor.
