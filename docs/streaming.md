...
| UI / effects          | Four open windows, 512 raster objects and 128 controls per surface; one resident/loading effect preview                                |
...
Click/focus the canvas. Arrows move/climb; Space jumps; Down+Space requests dropping; X/left Control holds attack. Up requests supported packaged portals before physics; I/E/S/K open UI windows, so S is not a gameplay-down alias. Held aliases are independent. Jump and Up presses latch across keyup until consumed or input clearing; an OS repeat cannot rearm a key cleared by focus/map replacement. Blur/visibility changes clear input. Attack does not invent unsupported combat or missing artwork.
