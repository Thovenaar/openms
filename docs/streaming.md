...
- Each region commits its complete entity set only after all of its required atlases are fetched, decoded and uploaded. Atlas references are per region, keyed globally by content hash; shared sources survive while any region/map candidate retains a reference.
...
Click/focus the canvas. Arrows or WASD move and climb; Space jumps; Down + Space requests dropping; X or left Control holds attack. Key aliases are tracked independently, jump press is an edge, and blur/visibility changes clear held input. Attack requests are routed to simulation but do not invent unsupported combat or missing artwork.
...
- `snapshot()`: allocates a JSON-safe inspection snapshot with `schemaVersion`, `buildId`, `currentMap`, packaged `maps`, `camera`, held `input`, `simulation`, `actorArtworkUnsupported`, resident `entities`, region readiness, `pendingLoads`, errors, `streaming` and bounded `metrics` arrays.
...
- `destroy()`: idempotently aborts map/neighbor demand, releases region/atlas/bitmap/worker resources, disconnects observers, removes input/control listeners and timers, and destroys Pixi.
...
Implementation is handed to the parent integration phase without running builds, lint, or tests during concurrent edits. Real keyboard movement, worker upload behavior, map transitions/cancellation, persistent-cache bounds and tiled-background appearance must be exercised against the integrated generated v2 catalog and the final physics modules. This document describes implemented policies, not a claim that those integrated browser scenarios have already passed.
