...
Click empty map space for keyboard play. Arrows move/climb; **Alt jumps**, Down+bound Jump requests eligible drop-through, Up enters supported portals, and a new **Control** press starts a local sword attack. Recovered defaults are I/E/S/K for Item/Equip/Stat/Skill, **Backslash Set Key, Q Quest, [ ShortCut and ] QuickSlot**. MiniMap remains a direct UI operation; the former C/H aliases were incorrect. Enter opens chat and accepted All-channel text produces a five-second **local, not-sent** speech bubble, not a fabricated received log. Escape closes the top window or opens the menu. KeyConfig edits a live nonmodal draft: parent OK persists, Cancel restores committed bindings. See the [full UI/input contract](ingame-ui.md).

Pause, reload, map/input guidance, Key bindings and audio controls are visible; scene/geometry, offline data and runtime details are collapsed advanced sections. Audio starts through **Enable audio (user gesture)**. Separate BGM/SE volume/mute controls affect original MP3 output, including accepted sword/mob cues. Damage digits and brief attacked-mob names use recovered presentation rules; damage decisions and mob AI remain explicitly local authority. NPC interaction/script boundaries and unavailable sound aliases remain explicit. The [scene contract](scene-contract.md) separates simulation, presentation and inspection.

Character, inventory/equipment, quests, location, audio preferences and committed **schema2 key bindings** checkpoint together in IndexedDB; valid v1 profiles migrate without changing gameplay state. Supported owned recovery items can be used by inventory double-click, bound key or quick slot, sharing a200ms throttle and consuming even at full vitals. No starter consumables or loot are fabricated. Corrupt/future saves and concurrent writers fail visibly instead of silently resetting or merging. The separate **Download complete release** action verifies the entire shell/catalog closure; **Use installed release and reload** then pins it for offline play. Browser storage eviction/site-data deletion remains outside the persistence guarantee.
...
- The [current systematic audit](client-audit.md) records25 native UI scenarios, independent chat/combat evidence, final defect replays,66 regressions/414 assertions and a410-check integrated pass. Earlier graphical/runtime reports are historical; neither browser agreement nor recovered artwork establishes original Windows fidelity.
