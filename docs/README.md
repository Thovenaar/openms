...
Click empty map space for keyboard play. Arrows move/climb; **Alt jumps**, Down+the bound Jump requests eligible drop-through, Up enters supported portals, and a new **Control** press starts a local sword attack. Recovered defaults are I/E/S/K for Item/Equip/Stat/Skill, **C KeyConfig, H MiniMap, Q Quest and ] QuickSlot**. Enter opens chat; Escape closes the top window or opens the menu. Space is Talk and X is Sit, not jump/attack aliases; unsupported original actions report their boundary. KeyConfig is nonmodal and edits a live draft: parent OK persists, Cancel restores committed bindings. Nearby NPC artwork opens data-backed quest dialogue. See the [full UI/input contract](ingame-ui.md) for graphical windows, quick keys, chat focus, cursor behavior and unsupported operations.
...
- The current graphical HUD/window/input correction has [native acceptance evidence](ingame-ui.md#graphical-ui-input-acceptance): 18 input scenario groups and an integrated 387-check pass. Earlier release measurements remain historical; neither browser agreement nor recovered artwork establishes original Windows fidelity.
