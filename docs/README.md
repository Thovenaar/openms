...
- [Current external-controls acceptance](validation.md#current-external-controls-acceptance) and the [console report](ingame-validation/console-simple/report.json): native draft/save/reload, failure recovery, keyboard navigation, responsive/DPR checks, and retained review corrections.
- [Current validation](validation.md#current-expanded-fidelity-acceptance) and the [expanded report](ingame-validation/expanded/report.json): exact source/asset identities, independent native playtests, retained failures/replays, server-stopped installed gameplay, world comparisons and147-test/876-assertion verification. The [skill contract](skills.md) distinguishes534 catalogued skills from35 supported controllers and499 explicitly unavailable entries.
