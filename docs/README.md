...
- [Inspection console and port tools](inspection-tools.md): four task-based Play/Character/Inspect/Settings tabs, preserved drafts, scoped XP/Windows95/Y2K themes, validated presets, bounded entity search and offline tools. Native in-game artwork remains unthemed. [Binding actions](offline-binding-actions.md) records exact native IDs and remote dependencies.
...
**Schema5** checkpoints character/AP/SP, instance-based inventory/equipment, appearance/base vitals, quests, location, settings/bindings, learned records, cash locker/gifts, Monster Book, macros and local social domains together in IndexedDB. Valid v1–v4 saves migrate without granting skills, points, cash or memberships. Native equip/use/drop/trade and supported NPC/quest/Cash actions use the shared atomic profile authority; multi-character exchanges commit all affected profiles together. The [persistent character editor](offline-profile.md) remains explicitly outside-game development policy. Complete-release installation pins verified assets separately from saves; site-data deletion/eviction remains outside the save guarantee.
...
- [Native offline UI acceptance](native-ui-validation.md) and [current validation](validation.md): exact source/asset identities, nine independent native browser slices, preserved failures and corrected replays, viewport/input/persistence evidence and current source gates.
...
Player follow now uses the recovered Shape2D filter (`0x1e0000`, coefficient100, 28-pixel deadband), retaining history across same-map teleport; interpolation between recovered30-ms simulation states remains browser presentation policy. Follow bounds use original VR rules; other viewport sizes generalize their half-extents. Whole compositions and camera positions project to signed integer world pixels before GPU submission. Manual camera inspection remains available. Background displacement/repetition and integer alpha are recovered, but absolute original startup attachment, special resize backgrounds, non-normal blends and original equal-z ties remain qualified. See the [camera-filter contract](ingame-portals.md#recovered-camera-filter-dynamics).
