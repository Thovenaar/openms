...
- Stage duration is `500 + 300 * abs(stageDelta)` milliseconds:1100ms for account→characters and characters→creation. `0043e7c7` reaches Shape2D `51408ee7`; `514092ec`, `51409688`, `51409860` and `51409d84` establish cubic Hermite evaluation. The starting velocity preserves nested integer truncation, `trunc(trunc((end-start)*7000/duration)/2)`, and terminal velocity is zero. The browser preserves the resulting small overshoot, not a CSS easing approximation. DLL evaluators and constants are retained in [Shape2D decompilation](ghidra-assets/Shape2D.dll/decompiled.c) and [interpolation evidence](ghidra-assets/Shape2D.dll/interpolation.txt).
- Account window origin `(10,-80)` becomes screen `(410,228)`; native login and registration controls use their original signboard artwork. Homepage deliberately opens `https://docs.openms.dev`, Register deliberately opens separate Windows95 account chrome, and saved-ID/recovery controls are deliberately absent.
...
- Explorer `00619119`, `006191c9` and `00618026` separate name and appearance phases through the field at `+0x238`; `005f7fe7` advances after name acceptance, `005f6920` returns to name entry and `005f7e7a` submits. The name panel begins at `(509,95)`; the preview feet are `(500,339)`. Appearance rows are face, hair, hair colour, skin, top, bottom, shoes, weapon and gender last. `006183d1` / `00618617` are Knight functions, not evidence for the Explorer layout. No dice step is introduced from unused archive artwork.
