...
- `bun client/tools/physics-inventory.js [--assets DIR] [--output FILE]` reproduces the metadata inventory. `bun client/tools/physics-map-probe.js [--assets DIR] [--output FILE]` reproduces extraction coverage. These are read-only original-data experiments, not build/test-suite commands.
