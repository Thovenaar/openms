...
- **Skills:** `0075f464` loads level `speed` and `jump` into secure fields at `+0x78` and `+0x84`; [skill instructions](ghidra-physics-options/skill-defaults.txt) retain the exact calls. Across player skill levels, `speed` has 229 occurrences, range -30..40; `jump` has 122 occurrences, range 1..20. There is no single name-derived rule for dash, teleport, rush, knockback, mounts, or job-specific skills. Those active application branches remain unsupported unless independently recovered by motion work.
