...
Use `bun server/tools/check-skill-effects.js --output /tmp/openms-local-feedback` for local chat/skill feedback and the loading indicator. It uses a disposable account at 500 ms RTT, holds chat replies for 1.5 s and cast replies for 3 s, and verifies local pending text, early artwork/audio and exact echo reconciliation. It also captures and checks the nonblocking indicator at 1280×800 and 800×600. The scenario waits for the chat settings debounce before its cast so an unrelated settings write does not change cast admission.
