...
The [current native-input report](ingame-validation/interaction-corrections/report.json) adds actual browser standing→prone receiver selection:44×65 from `00af14b8`, then46×31 from `00af14c8`. Real contact produced both ±200 horizontal/−200 vertical impulses and60-ms positive-hit tint phases; an ordinary nonfatal Red Snail hit used `hit1`. These checks do not cover every morph/mount/ladder/skill receiver or replace missing original Windows captures.
