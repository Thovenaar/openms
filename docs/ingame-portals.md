...
- **Same-map non-type-4/5 motion:** relocation occurs on the next eligible update without scene teardown or global fade. It synchronizes previous/current positions at the exact named `(x,y−10)` arrival, projects the destination pose and camera before drawing, and preserves the field systems, fixed clock and BGM owner. The original `Effect/BasicEff.img/Teleport` is **four80-ms frames,320ms total**; it is not the600-ms admission recovery. The provisional120-ms guard is not a travel pause. Types4/5 retain the packet/field path even when map IDs match.
- **Camera:** `00437a31`, with `00437b32/00437b35`, establishes the local-user target `(x,y−50)` before clipping. `00641ef1` uses center limits `VRLeft+400`, `VRRight−400`, `VRTop+300`, `VRBottom−300` at800×600, collapsing nonpositive ranges to their midpoint. The browser generalizes these limits to its viewport and places the camera at the destination before reveal; it never interpolates from source coordinates through intermediate map positions. Native vector slot`0xa8` follow dynamics are not reconstructed: immediate browser follow is explicitly retained rather than an invented tween.
...
The current [interaction report](ingame-validation/interaction-corrections/report.json) records native Henesys↔Maya travel, black-boundary replacement, the600-ms reveal with admitted movement, all four same-map Teleport frames without a global fade, and revival through the same field transition owner. A real portal request with an intentionally failed503 manifest retained Henesys and reversed opacity from0.392 to0; service-worker bypass made the network fault effective. A separate native map selection superseded a portal at opacity0.224, leaving zero pending loads and no stale black overlay. These are controlled browser failure tests, not original server behavior.
...
The investigation executed against all original Reactor/Morph/TamingMob images and the complete supported map graph; retained decompilation includes `entities-motion.txt`, `reactor-events.txt`, `reactor-triggers.txt`, `reactor-hit.txt`, and `reactor-layers.txt`. These are evidence runs, not browser proof. Final integrated source validation passed format, strict lint, 140 tests / 822 assertions and extraction. The earlier native evidence below covers its recorded implementation; the current interaction report above supplies the newer fade/failure acceptance, not exhaustive type3/type11 or all356-map gameplay coverage.
