...
- [Fresh Shape2D recovery](original-resource-audit.md#item-rotation-client-call-through-shape2d) follows that virtual call to the DLL's periodic angle evaluator. Drawing now includes elapsed visual time between fixed updates: offline uses its remainder, and online advances at most one 90ms publication interval. Reading only the network sample angle caused 108-degree jumps. Server landing and field pause still control when the spin stops.
