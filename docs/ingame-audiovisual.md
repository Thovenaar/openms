...
- `Game/DropItem` follows drop settlement, gated strictly **more than300ms** since the previous settlement cue in the drop owner. `Game/PickUpItem` follows successful durable local pickup before its collection presentation; failed/busy/dead/out-of-range pickup does not play. [Drop provenance](ghidra-client-corrections/drop-provenance.json) retains native settlement and local-owner-removal consumers. Durable-commit scheduling and the launch/collection curves are browser authority/presentation policies, not recovered native timing parity.
...
The [current native-input report](ingame-validation/interaction-corrections/report.json) observes real Web Audio source starts during button, carry and pickup input. Admitted pointer release and focused Space each produced one original click; off-target release produced no click. Carry start/admitted placement used the recovered DragStart/DragEnd samples. Successful real-item pickup produced one source start, while failed durable pickup retained the item. The observer did not synthesize playback; exact original Windows mixing, BGM crossfade and subjective sound parity remain unverified.
