...
Click empty map space to focus gameplay. Arrows move/climb; Space jumps; Down+Space requests eligible drop-through; Up requests supported local portal traversal; a new X/Control press requests a local sword attack. I/E/S/K open windows, with S reserved for Stat. Nearby NPC artwork invokes the data-backed quest owner; the journal exposes persistent progress. Browser camera/debug/pause/step controls remain inspection. Audio requires a native enable gesture. Unsupported scripts, skills and combat capabilities are exposed, not substituted.
