...
Runtime asset downloads use a 30-second idle deadline and five-minute total deadline, including response headers and streamed body reads. Nonempty chunks refresh only the idle deadline. Timeout aborts the request and releases its fetch gate even if the response stream fails to settle cancellation; caller cancellation remains an `AbortError`, timeout a `TimeoutError`. Existing byte, concurrency and queue ceilings remain unchanged. These are browser delivery policies, not recovered game constants. `client/test/stream-network.test.js` covers stalled headers/body, trickling responses, successful bounded reads and cancellation.
