...
`server:prod` forces production mode, including when `OPENMS_MODE=development` is inherited. Configure `DATABASE_URL`, the exact `OPENMS_ORIGIN`, and the reviewed `OPENMS_RULES_HASH` before starting. Set `OPENMS_STUDIO_ORIGIN` to Studio's exact origin, or an empty value to disable Studio access. Origins stay exact in every mode and HTTP is accepted, since TLS may terminate at the reverse proxy; set `OPENMS_ORIGIN` to the origin the browser actually sends. This command checks migrations and starts the server without creating or resetting development accounts.
...
A slim 24px project bar appears above the game in both development and production. X and GitHub open `x.com/tensorfish` and the repository in separate tabs; Docs opens `docs.openms.dev`. Bug Report opens an email to `tensorfish@proton.me`; About shows a short game summary and an as-is, use-at-your-own-risk disclaimer. Close the dialog with its button or Escape.
