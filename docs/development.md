...
The disk cache retains up to 4 GiB / 32,768 files, adapting to browser quota with a 192 MiB fallback when quota estimates are unavailable. This lets later map visits keep more downloaded files without enlarging decoded CPU/GPU limits. Existing cached files survive the upgrade. The client requests browser persistence without holding up startup; browser approval is optional. See the [cache policy and diagnostics](streaming.md#persistent-asset-cache).
