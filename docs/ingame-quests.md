...
The life owner's native record carries **`canInteract(): boolean`**, not a second admission alias. Quest UI rechecks that exact live closure before opening a conversation, advancing/answering, or committing acceptance/completion. A missing closure, stale field, hidden/unresident placement, dead player or out-of-range NPC cannot authorize the transaction. The numerical proximity policy belongs to LifeSystem and is not presented as a recovered original hitbox rule. Inspector selection does not authorize gameplay.
