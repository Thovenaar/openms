...
`profile-validation.js` is the structural save authority. Schema 5 keeps AP, ten SP pools, learned/master/expiration records and committed bindings, and adds the domains used by the integrated offline systems:
...
Migration validates legacy root keys and retained values before conversion. Version 1 receives recovered default bindings; v1/v2 receive ten zero SP pools, no learned ranks and default chat preferences; pre-v4 receives zero AP. All v1–v4 saves migrate to schema 5: aggregate inventory becomes individually identified, template-limited stacks; equipped template IDs receive unoccupied authored slots; base vitals preserve old maxima; new cash/book/social/macro domains are empty/default. Existing character progress, AP/SP where present, bindings, quests, location and audio/chat preferences survive. A legacy category requiring more slots retains that capacity as browser compatibility policy, **not** a purchased expansion. Missing required item metadata, conflicting equipped slots or an unrepresentable stack expansion reject migration without replacing the durable save. There are no migration skill, point, level, job or cash grants.
...
The sections above describe current schema-5 implementation, not a claim that every field and failure boundary has received final native replay. [Persistence evidence](offline-saves.md#executed-verification) distinguishes older schema/transaction observations from current coverage. Final build identity and consolidated acceptance belong to [validation](validation.md).
