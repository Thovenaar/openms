...
`createSimulation(world,{x,y})`, `advanceSimulation(sim,input,elapsedMs)`, and `snapshotSimulation(sim)` implement the reconstruction contract. Input uses held booleans `left,right,up,down,jump,attack` and one `jumpPressed` edge. The edge is consumed once on the next tick; an ineligible edge is rejected rather than buffered until a future landing. Holding jump does not fabricate repeated jump edges. Left/right and up/down cancel when both are held.

State strings are `ground`, `air`, `ladder`, `swim`, `fly`; `crouching` is a separate physical-pose boolean. Actions are `stand1`, `walk1`, `jump`, `prone`, `ladder`, `rope`. Buoyant airborne poses currently request available `jump` artwork; this is a presentation mapping, not proof of the original full action-selection state machine. Facing is numeric: `-1` left, `+1` right. Attack input sets `diagnostics.unsupportedAttack`; combat motion/action locks and damage are not invented.
...
5. Equipment, temporary stats, skills, mounts, transformations, knockback, special forced-motion modes and combat action locks are outside the base-avatar input contract. Their available metadata is inventoried but not silently applied or approximated. See [physics-options.md](physics-options.md).
...
Main owns integrated lint/build/tests and actual browser keyboard/presentation validation after concurrent edits settle.
