...
test("default migration inventory matches the runtime schema requirement", async () => {
  ...
  expect(migrations.at(-1).filename).toBe("008-item-history.sql");
});
...
async function checkFreshDatabase({ sql, url }) {
  ...
  expect(first.applied).toHaveLength(DATABASE_SCHEMA_VERSION);
  ...
  expect(second.skipped).toHaveLength(DATABASE_SCHEMA_VERSION);
...
async function checkAtomicFailure({ sql, url }) {
  ...
  expect((await sql`SELECT count(*)::int AS n FROM migrations`)[0].n).toBe(
...
async function checkLegacyDatabase({ sql, url }) {
  ...
  expect(runs.flatMap((run) => run.applied)).toHaveLength(
    DATABASE_SCHEMA_VERSION,
  );
  expect(runs.flatMap((run) => run.skipped)).toHaveLength(
    DATABASE_SCHEMA_VERSION,
  );
