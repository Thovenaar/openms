...
export class AuthoritySkillResources extends SkillResources {
  ...
  sourceIcon(kind, id) {
    const template = kind === "item"
      ? this.world.content.items[id]
      : this.world.content.catalog.ui.skills[id];
    const path = kind === "item" && Math.trunc(id / 10000) !== 238
      ? template?.iconRawPath : template?.iconPath;
  ...
  view(animation) {
    ...
    if (mount) {
      ...
      view.riding = {
        ...
        saddle: mount.saddleId ? catalog.saddles[mount.saddleId][mount.id].bundle : null,
        anchor: mount.anchor,
      };
    }
    return view;
  ...
  animationBundle(animation, source, mount) {
    const bundle = mount?.descriptor.bundle ??
      source?.bundle ?? animation.sources.get(animation.action);
    if (!bundle)
